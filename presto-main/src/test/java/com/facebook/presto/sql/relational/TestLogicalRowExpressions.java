...
public class TestLogicalRowExpressions
{
    ...
    private final RowExpression a = name("a");
    private final RowExpression b = name("b");
    private final RowExpression c = name("c");
    private final RowExpression d = name("d");
    private final RowExpression e = name("e");
    private final RowExpression f = name("f");
    private final RowExpression g = name("g");
    private final RowExpression h = name("h");
    ...
    public void testEliminateConstant()
    {
        ...
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(or(and(TRUE_CONSTANT, a), and(FALSE_CONSTANT, b))),
                a);
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(or(and(TRUE_CONSTANT, a), and(FALSE_CONSTANT, b))),
                a);
    ...
    public void testEliminateDuplicate()
    {
        ...
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(or(and(TRUE_CONSTANT, a), and(b, b))),
                or(a, b));

        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(or(and(a, b), and(a, b))),
                and(a, b));
        // we will prefer most simplified expression than correct conjunctive/disjunctive form
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(or(and(a, b), and(a, b))),
                and(a, b));

        // eliminate duplicated items with different order, prefers the ones appears first.
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(or(and(b, a), and(a, b))),
                and(b, a));
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(or(and(b, a), and(a, b))),
                and(b, a));

        // (b && a) || a
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(or(and(b, a), a)),
                a);
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(or(and(b, a), a)),
                a);
        // (b || a) && a
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(and(a, or(b, a))),
                a);
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(and(a, or(b, a))),
                a);

        // (b && a) || (a && b && c) -> b && a
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(or(and(b, a), and(and(a, b), c))),
                and(b, a));
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(or(and(b, a), and(and(a, b), c))),
                and(b, a));

        // (b || a) && (a || b) && (a || b || c || d) || (a || b || c) -> a
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(and(or(b, a), and(or(a, b), and(or(or(c, d), or(a, b)), or(a, or(b, c)))))),
                or(b, a));

        // (b || a)  && (a || b || c) && (a || b) && (a || b || nd) && e
        // we cannot eliminate nd because it is non-deterministic
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(and(and(or(b, a), and(or(a, or(b, c)), and(or(a, b), or(or(a, b), nd)))), e)),
                and(and(or(b, a), or(or(a, b), nd)), e));
        // we cannot convert to disjunctive form because nd is non-deterministic
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(and(and(or(b, a), and(or(a, or(b, c)), and(or(a, b), or(or(a, b), nd)))), e)),
                and(and(or(b, a), or(or(a, b), nd)), e));

        // (b || a)  && (a || b || c) && (a || b) && (a || b || d) && e
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(and(and(or(b, a), and(or(a, or(b, c)), and(or(a, b), or(or(a, b), d)))), e)),
                and(or(b, a), e));
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(and(and(or(b, a), and(or(a, or(b, c)), and(or(a, b), or(or(a, b), d)))), e)),
                or(and(b, e), and(a, e)));

        // (b || a || c) && (a || b || d) && (a || b || e) && (a || b || f)
        // already conjunctive form
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(and(or(or(b, a), c), and(or(d, or(a, b)), and(or(or(a, b), e), or(or(a, b), f))))),
        ...
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(and(or(or(b, a), c), and(or(d, or(a, b)), and(or(or(a, b), e), or(or(a, b), f))))),
    ...
    public void testConvertToCNF()
    {
        ...
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(
        ...
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(or(and(a, and(b, c)), and(and(d, e), f))),
    ...
    public void testBigExpressions()
    {
        ...
        RowExpression bigExpressionWithCommonPredicate = or(IntStream.range(0, 10001)
                ...
                .toArray(RowExpression[]::new));
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(bigExpressionWithCommonPredicate),
        ...
        assertEquals(logicalRowExpressions.convertToConjunctiveNormalForm(or(a, bigExpressionWithCommonPredicate)),
                or(Stream.concat(Stream.of(name("a")), IntStream.range(0, 10001).boxed().map(i -> and(name("a"), name("b" + i)))).toArray(RowExpression[]::new)));
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(or(a, bigExpressionWithCommonPredicate)),
                or(Stream.concat(Stream.of(name("a")), IntStream.range(0, 10001).boxed().map(i -> and(name("a"), name("b" + i)))).toArray(RowExpression[]::new)));
    }
    ...
    public void testConvertToDNF()
    {
        ...
        // (a && b && c) || ( d && e) will expand to too much
        assertEquals(logicalRowExpressions.convertToDisjunctiveNormalForm(and(or(a, or(b, c)), or(or(d, e), f))),
    ...
    private RowExpression and(RowExpression... expressions)
    {
        return logicalRowExpressions.and(expressions);
    }
