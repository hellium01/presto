            ...
            extends PlanVisitor<PlanWithProperties, PreferredProperties>
    {
        ...
        public PlanWithProperties visitTableWriter(TableWriterNode node, PreferredProperties preferredProperties)
        {
            ...
            if (!partitioningScheme.isPresent()) {
                if (scaleWriters) {
                    partitioningScheme = Optional.of(new PartitioningScheme(Partitioning.create(SCALED_WRITER_DISTRIBUTION, ImmutableList.of(), types.allTypes()), source.getNode().getOutputSymbols()));
                }
                else if (redistributeWrites) {
                    partitioningScheme = Optional.of(new PartitioningScheme(Partitioning.create(FIXED_ARBITRARY_DISTRIBUTION, ImmutableList.of(), types.allTypes()), source.getNode().getOutputSymbols()));
                }
            }
        ...
        public PlanWithProperties visitUnion(UnionNode node, PreferredProperties parentPreference)
        {
            ...
            if (!partitionedChildren.isEmpty() && unpartitionedChildren.isEmpty()) {
                ...
                result = new ExchangeNode(
                        ...
                        GATHER,
                        REMOTE,
                        new PartitioningScheme(Partitioning.create(SINGLE_DISTRIBUTION, ImmutableList.of(), types.allTypes()), node.getOutputSymbols()),
                        partitionedChildren,
                ...
                if (!partitionedChildren.isEmpty()) {
                    ...
                    result = new ExchangeNode(
                            ...
                            GATHER,
                            REMOTE,
                            new PartitioningScheme(Partitioning.create(SINGLE_DISTRIBUTION, ImmutableList.of(), types.allTypes()), exchangeOutputLayout),
                            partitionedChildren,
                            partitionedOutputLayouts,
                            Optional.empty());
            ...
            if (countSources(partitionedChildren) == 0) {
                ...
                return new PlanWithProperties(
                        new ExchangeNode(
                                ...
                                REPARTITION,
                                REMOTE,
                                new PartitioningScheme(Partitioning.create(FIXED_ARBITRARY_DISTRIBUTION, ImmutableList.of(), types.allTypes()), node.getOutputSymbols()),
                                partitionedChildren,
                                partitionedOutputLayouts,
                                Optional.empty()));
            }
        ...
        private Partitioning createPartitioning(List<Symbol> partitioningColumns)
        {
            ...
            if (GlobalSystemConnector.NAME.equals(partitioningProviderCatalog)) {
                return Partitioning.create(FIXED_HASH_DISTRIBUTION, ImmutableList.copyOf(partitioningColumns), types.allTypes());
            }
            ...
            return Partitioning.create(partitioningHandle, partitioningColumns, types.allTypes());
        }
    }
