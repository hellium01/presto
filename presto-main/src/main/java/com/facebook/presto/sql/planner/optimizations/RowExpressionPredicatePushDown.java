...
import com.facebook.presto.sql.planner.SubRowExpressionExtractor;
        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<RowExpression>
    {
                ...
                Session session)
        {
            ...
            this.logicalRowExpressions = new LogicalRowExpressions(determinismEvaluator, new FunctionResolution(metadata.getFunctionManager()));
        ...
        public PlanNode visitExchange(ExchangeNode node, RewriteContext<RowExpression> context)
        {
            ...
            for (int i = 0; i < node.getSources().size(); i++) {
                Map<VariableReferenceExpression, RowExpression> outputsToInputs = new HashMap<>();
        ...
        public PlanNode visitWindow(WindowNode node, RewriteContext<RowExpression> context)
        {
            ...
            Predicate<RowExpression> isSupported = conjunct ->
                    determinismEvaluator.isDeterministic(conjunct) &&
                            VariablesExtractor.extractUnique(conjunct)
                                    .stream()
        ...
        private boolean isInliningCandidate(RowExpression expression, ProjectNode node)
        {
            ...
            verify(SubRowExpressionExtractor.extract(expression)
                    .stream()
        ...
        {
            Map<VariableReferenceExpression, RowExpression> commonGroupingVariableMapping = node.getGroupingColumns().entrySet().stream()
        ...
        public PlanNode visitJoin(JoinNode node, RewriteContext<RowExpression> context)
        {
            ...
            if (newJoinPredicate.equals(FALSE_CONSTANT)) {
                newJoinPredicate = standardExpressions.compare(OperatorType.EQUAL, constant(0L, BIGINT), constant(1L, BIGINT));
            }
        ...
        private InnerJoinPushDownResult processInnerJoin(RowExpression inheritedPredicate, RowExpression leftEffectivePredicate, RowExpression rightEffectivePredicate, RowExpression joinPredicate, Collection<VariableReferenceExpression> leftVariables)
        {
            ...
            // Generate equality inferences
            RowExpressionEqualityInference allInference = new RowExpressionEqualityInference.Builder(functionManager, typeManager)
                    .addEqualityInference(inheritedPredicate, leftEffectivePredicate, rightEffectivePredicate, joinPredicate)
                    .build();
            RowExpressionEqualityInference allInferenceWithoutLeftInferred = new RowExpressionEqualityInference.Builder(functionManager, typeManager)
                    .addEqualityInference(inheritedPredicate, rightEffectivePredicate, joinPredicate)
                    .build();
            RowExpressionEqualityInference allInferenceWithoutRightInferred = new RowExpressionEqualityInference.Builder(functionManager, typeManager)
            ...
            for (RowExpression conjunct : new RowExpressionEqualityInference.Builder(functionManager, typeManager).nonInferrableConjuncts(inheritedPredicate)) {
            ...
            for (RowExpression conjunct : new RowExpressionEqualityInference.Builder(functionManager, typeManager).nonInferrableConjuncts(rightEffectivePredicate)) {
            ...
            for (RowExpression conjunct : new RowExpressionEqualityInference.Builder(functionManager, typeManager).nonInferrableConjuncts(leftEffectivePredicate)) {
            ...
            for (RowExpression conjunct : new RowExpressionEqualityInference.Builder(functionManager, typeManager).nonInferrableConjuncts(joinPredicate)) {
        ...
        {
            return standardExpressions.compare(OperatorType.EQUAL, equiJoinClause.getLeft(), equiJoinClause.getRight());
        }
        ...
        private boolean canConvertOuterToInner(List<VariableReferenceExpression> innerVariablesForOuterJoin, RowExpression inheritedPredicate)
        {
            ...
            for (RowExpression conjunct : extractConjuncts(inheritedPredicate)) {
                if (determinismEvaluator.isDeterministic(conjunct)) {
                    ...
                    if (response == null ||
                            (response instanceof ConstantExpression && ((ConstantExpression) response).getValue() == null) ||
                            FALSE_CONSTANT.equals(response)) {
        ...
        private Predicate<RowExpression> joinEqualityExpression(final Collection<VariableReferenceExpression> leftVariables)
        {
            return expression -> {
                ...
                if (determinismEvaluator.isDeterministic(expression) && standardExpressions.isComparison(expression)) {
                    if (standardExpressions.isOperation(expression, OperatorType.EQUAL)) {
        ...
        private PlanNode visitNonFilteringSemiJoin(SemiJoinNode node, RewriteContext<RowExpression> context)
        {
            ...
            RowExpressionEqualityInference inheritedInference = new RowExpressionEqualityInference.Builder(functionManager, typeManager)
            ...
            RowExpressionEqualityInference.EqualityPartition equalityPartition = inheritedInference.generateEqualitiesPartitionedBy(in(node.getSource()
        ...
        private PlanNode visitFilteringSemiJoin(SemiJoinNode node, RewriteContext<RowExpression> context)
        {
            ...
            RowExpression joinExpression = standardExpressions.compare(OperatorType.EQUAL, node.getSourceJoinVariable(), node.getFilteringSourceJoinVariable());
        ...
        public PlanNode visitUnnest(UnnestNode node, RewriteContext<RowExpression> context)
        {
            ...
            // Add the equality predicates back in
            RowExpressionEqualityInference.EqualityPartition equalityPartition = equalityInference.generateEqualitiesPartitionedBy(in(node.getReplicateVariables())::apply);
            pushdownConjuncts.addAll(equalityPartition.getScopeEqualities());
